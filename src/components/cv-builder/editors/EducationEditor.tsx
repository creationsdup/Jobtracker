...
export function EducationEditor({ libraryItems, selectedIds, onToggle, loading }: Props) {
  const items = libraryItems.filter(e => e.type === 'EDUCATION')
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-8 px-4">
        ...
        <p className="text-sm font-medium text-[var(--color-muted)]">Aucune formation dans la bibliothèque</p>
        <p className="text-xs text-gray-400 mt-1 mb-4">Ajoutez vos formations dans l'onglet Expériences (type Formation).</p>
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {items.map(edu => {
        const selected = selectedIds.includes(edu.id)
        return (
          <button
            key={edu.id}
          ...
          >
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold text-[var(--color-ink)] truncate">{edu.title}</p>
              <p className="text-[11px] text-[var(--color-muted)] truncate">{edu.organization}</p>
              <p className="text-[10px] text-gray-400 mt-0.5">
                {formatCVDate(edu.startDate)} – {edu.current ? 'Présent' : edu.endDate ? formatCVDate(edu.endDate) : ''}
              </p>
            </div>
          </button>
        )

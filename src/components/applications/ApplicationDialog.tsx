...
interface ApplicationDialogProps {
  ...
  aside?: ReactNode
  /** Élargit la colonne de droite sur grand écran (fiche : choix d'étape ouvert à côté de la timeline). */
  wideAside?: boolean
  footer?: ReactNode
...
export function ApplicationDialog({ title, onClose, children, aside, wideAside = false, footer, onSubmit, onKeyDown, overlays, appear = true }: ApplicationDialogProps) {
  const titleId = useId()
  const panelClass = cn(PANEL_CLASS, !aside ? 'sm:max-w-[560px]' : wideAside ? 'sm:max-w-[1320px]' : 'sm:max-w-[1000px]')
  ...
  const content = (
    <>
      ...
      {aside ? (
        ...
        <div
          className={cn(
            'flex-1 min-h-0 overflow-y-auto md:overflow-hidden md:grid md:grid-cols-[minmax(0,1fr)_minmax(320px,380px)]',
            wideAside && 'lg:grid-cols-[minmax(0,1fr)_minmax(620px,680px)]',
          )}
        >

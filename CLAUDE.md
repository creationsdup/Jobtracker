...
- Règles : ne jamais stocker ni journaliser un code en clair côté serveur ; sur l'appareil, seul `src/lib/savedAccessCode.ts` le garde (localStorage `jobtracker-board-code`, lié à l'id du tableau, effacé quand l'écran d'accès s'affiche, par « Quitter » et par la suppression) pour l'afficher masqué dans « Mon tableau » ; ne jamais afficher l'email technique ; pour supprimer un tableau, `board-delete` (pas `delete-account`, qui laisse les lignes keyées par `userId` texte).
